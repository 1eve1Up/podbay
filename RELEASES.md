...
- **End-to-end partial-deploy agent loop** — **`examples/ci-partial-agent-loop-demo.sh`** runs **`happy`** (partial **`deploy`** with **`--dependents`**, then **`diff`**, **`logs`**, **`down`** on the same roots) and **`fail`** (structured **`deploy_health_*`** issues, then **`logs`** / **`explain`** / **`down`**). [docs/agent-loop.md](docs/agent-loop.md) documents the gate sequence and failure playbook. No new CLI JSON shapes.
...
- **Compose `include:` (v1 subset)** — local relative paths only; merged before `extends:` in `podbay import compose`. See [docs/contract.md](docs/contract.md) (Import from Compose) and `examples/compose-include/`.
- **`podbay import compose --json`** — on failure, emits a versioned JSON envelope (`kind: import_compose`) with stable `issues[].code` values for compose read/parse, include graph errors, and unsupported include shapes; on success, emits one JSON document (`status: ok`) with `contract_yaml`, `service_count`, optional `project`, and optional `output_path` when `-o` is used. See [docs/contract.md](docs/contract.md) and [docs/cli-json.md](docs/cli-json.md).
...
- **Bidirectional `depends_on` / `dependents:`** — every child→parent edge appears on the parent's **`dependents`** list; validate rejects missing or stray entries. See [docs/contract.md](docs/contract.md).
- **`podbay teardown` / `podbay down` partial selection** — optional service roots and **`--dependents`** match **`validate`** / **`deploy`**; partial teardown skips project network removal while labelled containers remain; **`--volumes`** is rejected with partial roots; **`teardown` / `down` `--json`** may include **`deploy_services`** and **`dependents_expand`**. See [docs/agent-loop.md](docs/agent-loop.md).
- **`podbay logs --json`** — one versioned JSON document per invocation (`kind: logs`); captures non-streaming `podman logs` output in **`log_body`**. **`--json`** cannot be combined with **`--follow`**. See [docs/cli-json.md](docs/cli-json.md).
