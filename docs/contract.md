...
| `podbay receipt <file>` | Read and validate a deploy receipt (human summary includes `deploy_id` / `contract_digest` / `status` when present). |
| `podbay receipt list <dir>` | Inventory receipt JSON files in a directory (newest first; `--json` → `kind: receipt_list`). |
