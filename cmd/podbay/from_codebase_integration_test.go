...
func TestDockerfileFromCodebaseFlow_initOnboardValidate(t *testing.T) {
	dir := t.TempDir()
...
func TestDockerfileFromCodebaseDemoScript_offline(t *testing.T) {
	modRoot, err := findModuleRoot()
	if err != nil {
		t.Fatal(err)
	}
	...
	script := filepath.Join(modRoot, "examples", "ci-dockerfile-from-codebase-demo.sh")
	cmd := exec.Command("bash", script)
	cmd.Env = append(os.Environ(), "PODBAY_BIN="+bin)
	out, err := cmd.CombinedOutput()
	if err != nil {
