...
## Implementation Status: v2026.5.0 Public Preview

`v2026.5.0` is the first public May 2026 release. It is usable for narrow Podman stacks, but the `podbay.yaml` contract is not yet 1.0-stable.
...
Planned / not shipped in `v2026.5.0`:
...
Planned / not shipped in `v2026.5.0`: causal diagnosis.
...
Current `v2026.5.0` behavior: factual runtime state, health probes, dependency context, and unexpected containers. It does not infer root cause.
