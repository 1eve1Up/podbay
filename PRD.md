...
- **`podbay import compose --json`:** versioned JSON (`kind: import_compose`) for both success and failure: success includes `contract_yaml`, `service_count`, optional `project`, and optional `output_path` when `-o` is set; failure uses stable `issues[].code` values for compose read/parse, include cycle/depth/escape/unsupported, and generic contract translation failures. See [README](README.md).
