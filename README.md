...
- Adopts existing Compose or Dockerfile-only projects with `podbay init --from-codebase` (Compose preferred, then Dockerfile stub: copy `EXPOSE` / `HEALTHCHECK`, do not invent published ports → first-pass `podbay.yaml` → orient) or explicit `podbay import compose` for Compose migration.
...
| [docs/blog-sprint-40-orientation-requirements.md](docs/blog-sprint-40-orientation-requirements.md) | Sprint 40: orientation requirements skim + Dockerfile instruction fill |
